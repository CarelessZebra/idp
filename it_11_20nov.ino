...
unsigned long lastNodeDetectTime = 0;
int ledState = LOW;
...
void field(){
  ...
  while(i < 8 and bigLoop[i] == 1){
    ...
    if (detecting == true and currentUltraSoundTime - previousUltraSoundTime > 1000){
      previousUltraSoundTime = currentUltraSoundTime;
      sensity_t = analogRead(ultraSoundPin);
      dist_t = sensity_t * MAX_RANG / ADC_SOLUTION;
    ...
    unsigned long currentNodeDetectTime = millis();
    if(((leftBackLine == HIGH or rightBackLine == HIGH) or (leftFrontLine == LOW and rightFrontLine == HIGH)) and currentNodeDetectTime - lastNodeDetectTime > 2000) {
      currentUltraSoundTime = millis();
      lastNodeDetectTime = currentNodeDetectTime;
      currentNodeDetectTime = millis();
      //Serial.print(currentNodeDetectTime);
