...
int returnPath[10] = {1,3,1,3,1,3,1,4,1,1};
int toStartOfField[7] = {1,1,1,4,1,1,3};
int savedPosition = 3;

bool gridMode = false; //set to false for isolated field testing
...
void field(){
  ...
  while(i < 15 and bigLoop[i] == 1){
    ...
    currentUltraSoundTime = millis();
    if (i == 8 and currentUltraSoundTime - previousUltraSoundTime > 500 and millis() - lastNodeDetectTime > 6000){
...
void collectBlockInField(int k){
  ...
  while(millis() - timee < timeToTurn180/2){
    blink();
    motor(3,200);
  }
  /*
  ...
  */
  ...
  delay(500);
  /*
  unsigned long bitOfFollow = millis();
  while(millis()-bitOfFollow < 1000){
    blink();
    lineFollow();
  }*/
  k=0;
