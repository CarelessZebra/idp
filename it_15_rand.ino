...
void return_block(int i){
  ...
  motor1->setSpeed(255);
  motor2->setSpeed(215);
  delay(3400); // GOES FORWARD TILL FULLY IN WHITE BOX
  motor(0,0);
  delay(500);
  ...
  leftBackLine = digitalRead(leftBackLinePin);
  rightBackLine = digitalRead(rightBackLinePin);
  while(leftBackLine or rightBackLine == 0){
    motor1->run(FORWARD);
    motor2->run(FORWARD);
    motor1->setSpeed(255);
    motor2->setSpeed(215); 
  ...
  leftBackLine = digitalRead(leftBackLinePin);
  rightBackLine = digitalRead(rightBackLinePin);
  // Add correction when hits white line of white box
...
if (leftFrontLine == LOW && rightFrontLine == LOW){}
