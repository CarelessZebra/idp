...
unsigned long timeOfFlightDistance = 45;
...
void grid(){
  ...
  while(i < 23 and smallLoop[i] == 1 and stop == false){
    ...
    if (sensor.getDistance() < timeOfFlightDistance and sensor.getDistance() > 0)
    {
      stop = true;
      motor(0,0);
      delay(2000);
...
void return_block(int i){
  ...
  delay(2300);
  motor(0,0);
  // for first stop in start/finish box, we turn on blue LED and wait 6 seconds (as required in task requirements)
  delay(1000);
